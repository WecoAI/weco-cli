"""Utility functions for the LangSmith setup wizard."""
...
def discover_functions(source_files: list[str]) -> dict:
    ...
    Targets: functions with an ``inputs`` parameter (convention for LangSmith targets).
    Evaluators: functions with ``(run, example)`` parameters (LangSmith evaluator convention).
